...
class ICS_Config:
    ...
    def remote_load(self):
        ini_config = ConfigParser.ConfigParser()
        ini_config.read(self.Current_Setting_Path)
        config_value = ICS_RDP_Tester.ICS_RDP_Tester.Ping_Config_Type(\
            host = ini_config.get("remote","host","127.0.0.1"), \
            timeout =ini_config.get("remote","timeout","1000"), \
            count = ini_config.get("remote","count","5"),  \
            packet_size =  ini_config.get("remote","packet_size","55") )
        return config_value
    ...
    def email_load(self):
        ...
        config_value = ICS_Email_Tester.ICS_Email_Tester.Config_Type( \
            host = ini_config.get("email","host","127.0.0.1"), \
